...
export interface DocumentMenuLabels {
  share: string;
  download: string;
  /** Por qué se descargó en vez de abrirse la hoja de compartir. */
  fallback: string;
}

/**
 * Compartir o guardar un documento, detrás de los tres puntos.
 *
 * La hoja de compartir del sistema es la única manera de darle un archivo a
 * otra aplicación desde la web, y solo existe en el teléfono. En lo demás el
 * archivo se descarga y se abre el chat, para que la persona lo adjunte, en
 * vez de decirle que su navegador no puede.
 *
 * Vive aquí y no en la pantalla del resumen porque lo mismo hace falta en el
 * historial de un préstamo: el comportamiento —la hoja, el respaldo, el aviso
 * que sobrevive al cierre del menú— cuesta de acertar y no se copia.
 */
export function DocumentMenu({
  ...
  url: string;
  fileName: string;
  /** Lo que se manda: un PDF o la imagen de un recibo. */
  mimeType?: string;
  /** Lo que va en el chat al lado del archivo. */
  message: string;
  labels: DocumentMenuLabels;
}) {
  ...
  return (
    <div ref={box} className="relative">
      <Button
        ...
        aria-expanded={open}
        onClick={() => {
          // Abrir el menú se lleva el aviso: ya se leyó, y tapaba lo que se
          // está mirando mientras siguiera ahí.
          setFallback(false);
          setOpen(!open);
        }}
      />
      ...
      {open ? (
        <div className="absolute right-0 z-20 mt-1 w-60 overflow-hidden rounded-xl border border-border bg-surface shadow-lg">
          ...
          >
            ...
            {busy ? es.payments.sharing : labels.share}
          </button>
          <button
            type="button"
